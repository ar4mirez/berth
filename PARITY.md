...
| `down <org>` | compose output | as `compose` | `compose down` | if `BIND_ADDR=tailscale` and Tailscale is down: prints `ccenv: BIND_ADDR=tailscale but tailscale is not up on this host`, then runs compose anyway with `BIND_ADDR=` empty (compose's default, 127.0.0.1), exit 0 | not started (p2) |
...
| unported subcommands | `berth fw <org> allow` (and the other writing `fw` subcommands) fail with `fw allow is not in berth yet (phase 3); use ccenv for now`, before any side effect | until phase 3; ccenv would first write the template |
| `--read-only` reads | `logs` doesn't create `home-config`/`quarantine`, and `fw show`/`presets` don't write a missing template | read-only means no writes; nothing else changes (checked by `TestParityReadOnly`) |
